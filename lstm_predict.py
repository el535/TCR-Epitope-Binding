...
pretrainedpath='E:\classes\CS\CS5787\project\models\VJCOV_LSTM2_FIXEDSPLIT_e10_l500_d0.1_lr0.001_decay1e-05_neg_pos5/62.pt'
...
#runname='cont_e{}_l{}_d{}_lr{}_decay{}_neg_pos{}'.format(
...
#recall
tn,fp,fn,tp=cov_conf=confusion_matrix(acts[cov_mask].cpu().numpy(),1*(preds[cov_mask]>0.5).cpu().numpy()).ravel()
...
tn,fp,fn,tp=cov_conf=confusion_matrix(acts[mcpas_mask].cpu().numpy(),1*(preds[mcpas_mask]>0.5).cpu().numpy()).ravel()
...
tn,fp,fn,tp=cov_conf=confusion_matrix(acts[vdj_mask].cpu().numpy(),1*(preds[vdj_mask]>0.5).cpu().numpy()).ravel()
...
tn,fp,fn,tp=cov_conf=confusion_matrix(acts.cpu().numpy(),1*(preds>0.5).cpu().numpy()).ravel()

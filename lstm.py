...
training_loader=BatchSampler(dftrain, neg_pos_ratio=neg_pos_ratio)
val_loader=BatchSampler(dfvalid, neg_pos_ratio=neg_pos_ratio)
test_loader=BatchSampler(dftest, neg_pos_ratio=neg_pos_ratio)
...
wandb.init(project="TCR_LSTM2", name='e{}_l{}_d{}_lr{}_decay{}_neg_pos{}_posw{}'.format(
    str(embedding_dim),str(lstm_dim),str(dropout),str(lr),str(wdecay),str(neg_pos_ratio),str(pos_weight)), reinit=True)
...
model = DoubleLSTMClassifier(32, 16, dropout=0.1, device=device).to(device)
